...
const portableTextComponents = {
	...
	block: {
		h2: ({ children }: any) => (
			<h2 className='text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900 mb-6 mt-12 leading-tight tracking-tight border-l-4 border-primary-red pl-5'>
				{children}
			</h2>
		),
		h3: ({ children }: any) => (
		...
		normal: ({ children }: any) => (
			<p className='text-gray-600 leading-[1.8] mb-6 text-lg md:text-xl font-light selection:bg-primary-red/10'>
				{children}
			</p>
		),
		blockquote: ({ children }: any) => (
			<blockquote className='border-l-4 border-primary-red pl-8 py-4 my-10 italic text-gray-700 bg-gray-50/50 rounded-r-2xl font-serif text-xl md:text-2xl'>
				{children}
			</blockquote>
		),
	},
	marks: {
		...
		link: ({ children, value }: any) => (
			<a
				...
				className='text-primary-red hover:underline decoration-2 underline-offset-4 transition-all'
			>
				{children}
			</a>
		),
	...
	listItem: {
		bullet: ({ children }: any) => (
			<li className='flex items-start gap-3 text-gray-600 text-lg md:text-xl leading-relaxed'>
				<span className='mt-2 w-1.5 h-1.5 rounded-full bg-primary-red shrink-0' />
				<span>{children}</span>
			</li>
		),
		number: ({ children }: any) => (
		...
		<article className='min-h-screen bg-gray-50/30'>
			...
			<div className='bg-white border-b border-gray-100 shadow-sm sticky top-0 z-50'>
				<div className='container mx-auto px-6 py-4'>
					...
					>
						<span className='p-2 rounded-full group-hover:bg-primary-red group-hover:text-white transition-all duration-300'>
			...
			<div className='relative w-full aspect-video md:h-[60vh] max-h-[800px] overflow-hidden bg-black'>
				...
				<div className='absolute bottom-0 left-0 w-full p-8 md:p-16 lg:p-24 z-10'>
					<div className='container mx-auto max-w-5xl text-center md:text-left'>
						<div className='inline-flex items-center gap-2 bg-primary-red text-white text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] px-4 py-2 rounded-full mb-6 shadow-2xl'>
						...
						<div className='flex flex-wrap items-center justify-center md:justify-start gap-8'>
							{post.author && (
								<div className='flex items-center gap-4 group'>
									<div className='relative w-14 h-14 rounded-full overflow-hidden ring-4 ring-white/10 group-hover:ring-primary-red/50 transition-all'>
										{post.author.image ? (
											<Image
											...
											<div className='w-full h-full bg-primary-red flex items-center justify-center text-white text-xl font-bold'>
				...
				<div className='max-w-4xl mx-auto'>
					...
					<div className='prose-container bg-white rounded-[2.5rem] shadow-2xl p-8 md:p-16 lg:p-24 border border-gray-100/50 relative'>
						...
						<div className='absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2'>
							<div className='bg-primary-red w-16 h-1 rounded-full' />
						</div>
						...
						<div className='mt-20 pt-16 border-t border-gray-100 text-center md:text-left'>
							<div className='flex flex-col md:flex-row items-center justify-between gap-10 bg-gray-50/50 p-10 rounded-3xl border border-gray-100'>
								...
								<button className='bg-gray-900 text-white px-8 py-4 rounded-2xl font-bold hover:bg-primary-red transition-all shadow-xl hover:-translate-y-1 shrink-0'>
									Join the Network
								</button>
							</div>
						</div>
						...
						<div className='mt-16 flex flex-col md:flex-row items-center justify-between gap-8 border-t border-gray-100 pt-10'>
							...
							<Link
								href='/news'
								className='group flex items-center gap-3 bg-black text-white px-10 py-4 rounded-2xl font-bold tracking-tight transition-all hover:bg-primary-red hover:shadow-2xl hoverShadow-primary-red/20 active:scale-95'
							>
								Back to Hub
								<Icon
									icon='mdi:newspaper'
